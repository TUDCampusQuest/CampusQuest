...
export default function TrailCaptureOverlay({
    captureMode, setCaptureMode, capturedPoints, setCapturedPoints, onClose, onTrailSaved
}) {
    ...
    const handleSaveToCloud = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Tell page.js to refetch immediately so the list is fresh
                // before the user even sees the home page
                if (onTrailSaved) onTrailSaved();

                // Show success toast then redirect home after 2 s
                setShowToast(true);
                setTimeout(() => {
                    setShowToast(false);
                    router.push('/');
        ...
        <>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                    <button onClick={handleUndo} disabled={capturedPoints.length === 0} style={{
                    ...
                    <button onClick={handleClear} disabled={capturedPoints.length === 0} style={{
                        flex: 1, padding: '8px', borderRadius: 8, border: '1px solid #ccc',
                        cursor: capturedPoints.length === 0 ? 'not-allowed' : 'pointer',
                ...
                <button onClick={handleOpenModal} disabled={capturedPoints.length === 0} style={{
                    ...
                    cursor: capturedPoints.length === 0 ? 'not-allowed' : 'pointer',
                    background: capturedPoints.length === 0 ? '#ccc' : '#1BA39C', color: '#fff',
                }}>
                    ...
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button onClick={handleSaveToCloud} disabled={isSaving} style={{
                                    flex: 1, padding: 10,
                                    background: isSaving ? '#aaa' : '#1BA39C',
