...
import dynamic from 'next/dynamic'; // FIX: Required for Mapbox in Next.js
...
// FIX: Dynamic Import (Prevents the white screen/hydration error)
...
export default function Home() {
  ...
  const handleToggle3D = () => {
    ...
    mapRef.current.easeTo({ pitch: currentPitch === 0 ? 60 : 0 });
  };

  // FIX: AWS Safety Guard - ensure locations is an array before filtering
  ...
  return (
    <Box sx={{ height: "100vh", width: "100vw", position: "relative", overflow: "hidden", bgcolor: "#f1f5f9" }}>
      ...
      {/* THE MAP COMPONENT */}
      {/* Container ensures map fills the space correctly */}
      ...
      <Stack spacing={1.5} sx={{ position: "absolute", right: 16, top: 100, zIndex: 1000 }}>
        ...
        <SideButton onClick={handleToggle3D}>
          ...
          <List sx={{ flex: 1, overflowY: "auto" }}>
            {filtered.map((loc) => (
              <ListItem 
                ...
                sx={{ mb: 1.5, borderRadius: "16px", border: "1px solid #f1f5f9", cursor: "pointer" }}
              >
                <ListItemText 
